...
function getItem(id){ return market.find(x=>x.id===id); }
...
async function initFirebase(){
  try{
    ...
    if(!cfg.firebaseConfig?.projectId) throw new Error("empty");
    const [{initializeApp},{getFirestore,collection,getDocs,doc,setDoc},{getAuth,signInAnonymously,onAuthStateChanged}]
      = await Promise.all([
        import("https://www.gstatic.com/firebasejs/11.10.0/firebase-app.js"),
        import("https://www.gstatic.com/firebasejs/11.10.0/firebase-firestore.js"),
        import("https://www.gstatic.com/firebasejs/11.10.0/firebase-auth.js")
      ]);
    const app=initializeApp(cfg.firebaseConfig), db=getFirestore(app), auth=getAuth(app);
    await signInAnonymously(auth);
    onAuthStateChanged(auth,async user=>{
      if(!user)return;
      window.__giftlabCloud={db,user,setDoc,doc};
      const snap=await getDocs(collection(db,"market"));
      if(!snap.empty){
        market=snap.docs.map(d=>({id:d.id,...d.data(),demo:false}));
        renderMarket();renderCases();renderUpgrade();
      }
      const pill=document.querySelector("#syncStatus");pill.classList.add("online");pill.innerHTML="<i></i> Firebase";
    });
  }catch(e){
    // expected in local demo
  }
}
let cloudTimer;
function syncCloud(){
